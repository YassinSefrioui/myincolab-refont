...
export default function Profile() {
  const { db, updateDB, user, setUser, prefs, setLang, setTheme, logout, logAudit, toast, t } = useApp();
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 160 }}>
            ...
            <select className="select" value={prefs.lang} onChange={e => setLang(e.target.value)}>
              {[['fr', 'Français'], ['en', 'English'], ['es', 'Español'], ['it', 'Italiano']].map(([k, lbl]) => <option value={k} key={k}>{lbl}</option>)}
            </select>
          </div>
          <div style={{ flex: 1, minWidth: 160 }}>
