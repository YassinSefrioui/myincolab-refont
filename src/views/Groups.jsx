...
function GroupCard({ g, db, updateDB, openModal, t, canEdit }) {
  ...
  return (
    <div className="card group-card" data-tour="groups-list">
      <div className="group-head">
        ...
        {canEdit && (
          <div style={{ display: 'flex', gap: 6 }} data-tour="groups-manage">
      ...
      {children.length > 0 && (
        <div className="subgroup-wrap">
          {children.map(c => <GroupCard key={c.id} g={c} db={db} updateDB={updateDB} openModal={openModal} t={t} canEdit={canEdit} />)}
        </div>
      )}
    </div>
  );
...
export default function Groups() {
  ...
  return (
    <div className="view-anim">
      <div className="boards-head" data-tour="groups-header">
      ...
      {roots.map(g => <GroupCard key={g.id} g={g} db={db} updateDB={updateDB} openModal={openModal} t={t} canEdit={canEdit} />)}
    </div>
  );
}
