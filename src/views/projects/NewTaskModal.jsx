...
export default function NewTaskModal({ project, colId = 'backlog' }) {
  ...
  function createTask() {
    ...
    updateDB(draft => {
      ...
      col.cards.push({ id, title: trimmed, assignee: assignee || user.id, label, due: due || null, priority: prio, comments: [], subtasks: [], deps: [], done: false });
    });
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <div style={{ flex: 1 }}>
          ...
          <MemberPicker
            candidates={project.members.map(id => db.team.find(x => x.id === id)).filter(Boolean)}
